"""
Book2Skills - Agent Skills Production Factory

Usage:
    book2skills [--workspace DIR] [--task TASK]

Examples:
    book2skills                              # Use current directory as workspace (interactive mode)
    book2skills --workspace /path/to/dir     # Use specific workspace directory (interactive mode)
    book2skills --task "generate skills from text"       # Execute a task non-interactively
"""
...
def get_log_directory() -> Path:
    ...
    return Path.home() / ".book2skills" / "log"
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Book2Skills - Agent Skills Production Factory",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  book2skills                              # Use current directory as workspace
  book2skills --workspace /path/to/dir     # Use specific workspace directory
  book2skills log                          # Show log directory and recent files
  book2skills log agent_run_xxx.log        # Read a specific log file
        """,
    )
    parser.add_argument(
        "--workspace",
...
async def run_agent(workspace_dir: Path, task: str = None):
    ...
    if not config_path.exists():
        ...
        print(f"  {Colors.DIM}1) config/config.yaml{Colors.RESET} (development)")
        print(f"  {Colors.DIM}2) ~/.book2skills/config/config.yaml{Colors.RESET} (user)")
        ...
        print(
            f"  {Colors.BRIGHT_GREEN}# Please create the config file manually in ~/.book2skills/config/config.yaml{Colors.RESET}"
        )
        print()
        print(f"{Colors.DIM}  This should include:{Colors.RESET}")
        print(f"{Colors.DIM}    • Create ~/.book2skills/config/{Colors.RESET}")
        print(f"{Colors.DIM}    • Add your API Key to config.yaml{Colors.RESET}")
        print()
        print(f"{Colors.BRIGHT_YELLOW}📝 Manual Setup:{Colors.RESET}")
        user_config_dir = Path.home() / ".book2skills" / "config"
    ...
    if system_prompt_path and system_prompt_path.exists():
        ...
        system_prompt = "You are Book2Skills-Agent, an intelligent assistant focused on converting professional knowledge into agent skills."
    ...
    history_file = Path.home() / ".book2skills" / ".history"
        ...
        try:
            ...
            if user_input.startswith("/"):
                ...
                if command in ["/exit", "/quit", "/q"]:
                    print(f"\n{Colors.BRIGHT_YELLOW}👋 Goodbye! Thanks for using Book2Skills{Colors.RESET}\n")
                    print_stats(agent, session_start)
                    break
